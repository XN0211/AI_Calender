...
class Calendar {
    ...
    selectDate(dateString) {
        ...
        const selectedDay = Array.from(document.querySelectorAll('.calendar-day')).find(day => {
            ...
            const date = this.getDateFromElement(day, dayNumber);
...
class AIAssistant {
    ...
    formatPlanResult(data) {
        ...
        result += '✨ All activities have been automatically added to your calendar!';
        return result;
    }
}
...
class CountdownManager {
    ...
    bindEvents() {
        document.getElementById('refresh-countdowns-btn').addEventListener('click', () => {
            this.loadCountdowns();
        });
        ...
        document.getElementById('export-countdowns-btn').addEventListener('click', () => {
            this.exportCountdowns();
        });
        ...
        document.getElementById('schedule-prep-btn').addEventListener('click', () => {
            this.showSchedulePrepModal();
        });
    }

    async loadCountdowns() {
        try {
            const response = await fetch('/get_countdowns');
            const data = await response.json();
            this.countdownData = data;
            
            this.renderCountdownStats(data.statistics);
            this.renderPriorityCards(data.statistics);
            this.renderCountdownList(data.countdowns);
            
            this.showCountdownResult('✅ Countdowns refreshed!');
        } catch (error) {
            console.error('Error loading countdowns:', error);
            this.showCountdownResult('❌ Error loading countdowns. Please try again.');
        }
    }

    renderCountdownStats(stats) {
        const statsContainer = document.getElementById('countdown-stats');
        
        statsContainer.innerHTML = `
            <div class="countdown-stat-card critical">
                <div class="countdown-stat-number">${stats.total}</div>
                <div class="countdown-stat-label">Total Deadlines</div>
            </div>
            <div class="countdown-stat-card critical">
                <div class="countdown-stat-number">${stats.critical}</div>
                <div class="countdown-stat-label">Critical (≤1 day)</div>
            </div>
            <div class="countdown-stat-card urgent">
                <div class="countdown-stat-number">${stats.urgent}</div>
                <div class="countdown-stat-label">Urgent (≤3 days)</div>
            </div>
            <div class="countdown-stat-card high">
                <div class="countdown-stat-number">${stats.high}</div>
                <div class="countdown-stat-label">High (≤7 days)</div>
            </div>
        `;
    }

    renderPriorityCards(stats) {
        ...
        const priorityData = [
            {
                ...
                description: 'Due today or tomorrow'
            },
            {
                priority: 'urgent',
                icon: '⚠️',
            ...
            },
            {
                ...
                icon: '📅',
                title: 'Medium',
                count: Math.max(0, stats.total - stats.critical - stats.urgent - stats.high),
                description: 'Due within 2 weeks'
            }
        ];
        
        priorityCards.innerHTML = priorityData.map(item => `
            <div class="priority-card ${item.priority}">
        ...
        if (countdowns.length === 0) {
            countdownList.innerHTML = `
                <div style="text-align: center; color: #666; padding: 40px;">
                    ...
                    <p>You're all caught up. Great job staying on top of your tasks!</p>
                </div>
            `;
            return;
        }

        // Filter countdowns based on current filter
        const filteredCountdowns = this.currentFilter === 'all' 
            ? countdowns 
            : countdowns.filter(item => item.priority === this.currentFilter);

        countdownList.innerHTML = filteredCountdowns.map(item => this.createCountdownItem(item)).join('');
        
        // Add event listeners to action buttons
        this.bindCountdownItemEvents();
    }
    ...
    createCountdownItem(item) {
        ...
        const actionButton = item.is_labeled 
            ? `<button class="countdown-action-btn unmark" data-date="${item.date}" data-activity="${item.activity}">Unmark Important</button>`
            : `<button class="countdown-action-btn mark" data-date="${item.date}" data-activity="${item.activity}">Mark Important</button>`;
        
        return `
            <div class="countdown-item ${item.priority}" data-priority="${item.priority}">
                <div class="countdown-header">
                    <div class="countdown-activity">${item.activity}</div>
                    <div class="countdown-timer">
                        <div class="countdown-days ${item.priority}">${item.days_remaining}</div>
                ...
                <div class="countdown-details">
                    ...
                    <div class="countdown-actions">
                        ${actionButton}
                        <button class="countdown-action-btn reminder" data-date="${item.date}" data-activity="${item.activity}">Add Reminder</button>
                    </div>
                </div>
                ${item.label ? `<div style="margin-top: 8px; font-size: 0.8rem; color: #666;">🏷️ ${item.label}</div>` : ''}
            </div>
        `;
    }

    bindCountdownItemEvents() {
        // Mark/Unmark important buttons
        document.querySelectorAll('.countdown-action-btn.mark').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const date = e.target.dataset.date;
                const activity = e.target.dataset.activity;
                this.markAsImportant(date, activity);
            });
        });

        document.querySelectorAll('.countdown-action-btn.unmark').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const date = e.target.dataset.date;
                this.unmarkAsImportant(date);
            });
        });

        // Add reminder buttons
        document.querySelectorAll('.countdown-action-btn.reminder').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const date = e.target.dataset.date;
                const activity = e.target.dataset.activity;
                this.addReminder(date, activity);
            });
        });
    }
    ...
    setFilter(filter) {
        ...
        // Re-render countdown list with filter
        if (this.countdownData) {
            this.renderCountdownList(this.countdownData.countdowns);
        }
    }
    ...
    toggleAutoRefresh() {
        ...
        if (this.isAutoRefreshEnabled) {
            ...
            this.isAutoRefreshEnabled = false;
            toggleBtn.innerHTML = '<i class="fas fa-play"></i> Auto Refresh';
            toggleBtn.classList.remove('btn-success');
            toggleBtn.classList.add('btn-secondary');
        } else {
            // Enable auto refresh (refresh every 5 minutes)
            this.autoRefreshInterval = setInterval(() => {
                this.loadCountdowns();
            }, 5 * 60 * 1000);
            this.isAutoRefreshEnabled = true;
            toggleBtn.innerHTML = '<i class="fas fa-pause"></i> Auto Refresh';
            toggleBtn.classList.remove('btn-secondary');
            toggleBtn.classList.add('btn-success');
        }
    }

    async markAsImportant(date, activity) {
        try {
            const response = await fetch('/mark_as_important', {
                method: 'POST',
                headers: {
            ...
            if (result.status === 'success') {
                this.showCountdownResult('✅ Event marked as important!');
                this.loadCountdowns(); // Refresh the list
            } else {
                this.showCountdownResult('❌ Error marking as important');
            }
        } catch (error) {
    ...
    async unmarkAsImportant(date) {
        try {
            const response = await fetch('/unmark_as_important', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            if (result.status === 'success') {
                this.showCountdownResult('✅ Important marking removed!');
                this.loadCountdowns(); // Refresh the list
            } else {
                this.showCountdownResult('❌ Error removing important marking');
            }
        } catch (error) {
    ...
    addReminder(date, activity) {
        // For now, just show a message. This could be expanded to integrate with notification systems
        this.showCountdownResult(`🔔 Reminder added for ${activity} on ${this.formatDate(date)}`);
    }

    exportCountdowns() {
        if (!this.countdownData) {
            this.showCountdownResult('❌ No countdown data to export');
            return;
        }

        const dataStr = JSON.stringify(this.countdownData, null, 2);
        const dataBlob = new Blob([dataStr], {type: 'application/json'});
        const url = URL.createObjectURL(dataBlob);
        
        const link = document.createElement('a');
        link.href = url;
        link.download = `countdowns_${new Date().toISOString().split('T')[0]}.json`;
        link.click();
        
        URL.revokeObjectURL(url);
        this.showCountdownResult('✅ Countdowns exported successfully!');
    }

    showMarkImportantModal() {
        // Simple modal for marking events as important
        const modal = document.createElement('div');
        modal.className = 'modal';
        modal.innerHTML = `
            <div class="modal-content">
                <h3>Mark Event as Important</h3>
                <p>Select an event from your calendar to mark as important:</p>
        ...
        // Populate events (simplified for now)
        const eventSelection = modal.querySelector('#event-selection');
        eventSelection.innerHTML = '<p style="color: #666;">Feature coming soon! Use the "Mark Important" button on individual countdown items.</p>';
    }

    showAddReminderModal() {
        this.showCountdownResult('🔔 Reminder feature coming soon!');
    }

    showSchedulePrepModal() {
        this.showCountdownResult('📅 Prep time scheduling feature coming soon!');
    }
    ...
    showCountdownResult(message) {
        const resultDisplay = document.getElementById('countdown-result');
        resultDisplay.textContent = message;
        resultDisplay.style.color = message.includes('✅') ? '#28a745' : 
                                  message.includes('❌') ? '#dc3545' : '#17a2b8';
...
// Auto-Display Countdown functionality
class AutoCountdownManager {
    constructor() {
        this.autoCountdownData = null;
        this.isVisible = true;
        this.autoRefreshInterval = null;
        this.bindEvents();
    ...
    bindEvents() {
        document.getElementById('toggle-countdown-btn').addEventListener('click', () => {
            this.toggleVisibility();
        });

        document.getElementById('refresh-auto-countdown-btn').addEventListener('click', () => {
            this.loadAutoCountdowns();
        });
    }

    async initializeAutoDisplay() {
        // Load countdowns immediately on page load
        await this.loadAutoCountdowns();
    ...
    async loadAutoCountdowns() {
        try {
            const response = await fetch('/get_countdowns');
            const data = await response.json();
            this.autoCountdownData = data;
            
            this.renderAutoCountdownDisplay(data);
        } catch (error) {
            console.error('Error loading auto countdowns:', error);
            this.renderAutoCountdownError();
        }
    }

    renderAutoCountdownDisplay(data) {
        const displayContainer = document.getElementById('auto-countdown-display');
        
        if (!data.countdowns || data.countdowns.length === 0) {
            displayContainer.innerHTML = `
                <div class="auto-countdown-empty">
        ...
        const importantCountdowns = data.countdowns.filter(item => 
            ['critical', 'urgent', 'high'].includes(item.priority)
        ).slice(0, 6); // Show max 6 items

        const countdownCards = importantCountdowns.map(item => this.createAutoCountdownCard(item)).join('');
        
        const summaryStats = this.createAutoCountdownSummary(data.statistics);
        
        displayContainer.innerHTML = `
            <div class="auto-countdown-grid">
                ${countdownCards}
            </div>
            ${summaryStats}
        `;
    ...
    createAutoCountdownCard(item) {
        ...
        return `
            <div class="auto-countdown-card ${item.priority}">
                <div class="auto-countdown-content">
                    <div class="auto-countdown-info">
                        <div class="auto-countdown-activity">${priorityIcon} ${item.activity}</div>
                        <div class="auto-countdown-date">${this.formatDate(item.date)}</div>
                        ${item.label ? `<div class="auto-countdown-label">${item.label}</div>` : ''}
                    </div>
                    <div class="auto-countdown-timer">
                        <div class="auto-countdown-days ${item.priority}">${item.days_remaining}</div>
                        <div class="auto-countdown-text">${daysText}</div>
                    </div>
                </div>
            </div>
        `;
    ...
    createAutoCountdownSummary(stats) {
        ...
        return `
            <div class="auto-countdown-summary">
                <div class="auto-countdown-summary-stats">
                    ${critical > 0 ? `<div class="auto-countdown-summary-stat critical">🚨 ${critical} Critical</div>` : ''}
                    ${urgent > 0 ? `<div class="auto-countdown-summary-stat urgent">⚠️ ${urgent} Urgent</div>` : ''}
                    ${high > 0 ? `<div class="auto-countdown-summary-stat high">⚡ ${high} High</div>` : ''}
                    <div class="auto-countdown-summary-stat">📅 ${total} Total</div>
                </div>
                <a href="#" class="auto-countdown-summary-link" onclick="switchToCountdownTab()">
    ...
    toggleVisibility() {
        const section = document.getElementById('auto-countdown-section');
    ...
    // Method to be called from summary link
    switchToCountdownTab() {
        // Switch to the countdown tab
        const countdownTab = document.querySelector('[data-tab="countdown"]');
        if (countdownTab) {
            countdownTab.click();
        }
    }
...
// Global function to switch to countdown tab
function switchToCountdownTab() {
    const countdownTab = document.querySelector('[data-tab="countdown"]');
    if (countdownTab) {
        countdownTab.click();
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    autoCountdownManager = new AutoCountdownManager();
